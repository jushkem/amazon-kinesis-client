...
public class HierarchicalShardSyncerTest {
    ...
    /**
     * Test determineNewLeasesToCreateWithPartialShardMap() where there are no shards
     */
    @Test
    public void testDetermineNewLeasesToCreateNoShards() {
        ...
        assertThat(HierarchicalShardSyncer.determineNewLeasesToCreateWithPartialShardMap(shards, leases, INITIAL_POSITION_LATEST).isEmpty(),
                equalTo(true));
    }

    /**
     * Test determineNewLeasesToCreateWithPartialShardMap() where there are no leases and no resharding operations have been performed
     */
    @Test
    public void testDetermineNewLeasesToCreate0Leases0Reshards() {
        ...
        final List<Lease> newLeases = HierarchicalShardSyncer.determineNewLeasesToCreateWithPartialShardMap(shards, currentLeases,
    ...
    /**
     * Test determineNewLeasesToCreateWithPartialShardMap() where there are no leases and no resharding operations have been performed, but
    ...
    public void testDetermineNewLeasesToCreate0Leases0Reshards1Inconsistent() {
        ...
        final List<Lease> currentLeases = Arrays.asList(newLease(shardId1));
        final Set<String> inconsistentShardIds = new HashSet<>(Collections.singletonList(shardId2));

        final List<Lease> newLeases = HierarchicalShardSyncer.determineNewLeasesToCreateWithFullShardMap(shards, currentLeases,
                INITIAL_POSITION_LATEST, inconsistentShardIds);
        final Set<String> newLeaseKeys = newLeases.stream().map(Lease::leaseKey).collect(Collectors.toSet());
        final Set<String> expectedLeaseShardIds = new HashSet<>(Arrays.asList(shardId0));
        assertThat(newLeases.size(), equalTo(expectedLeaseShardIds.size()));
        assertThat(newLeaseKeys, equalTo(expectedLeaseShardIds));
    }
    ...
    /**
     * Test checkAndCreateLeasesForNewShardsWithFullShardMap while not providing a pre-fetched list of shards
     */
    @Test
    public void testCheckAndCreateLeasesForShardsIfMissingAtLatest() throws Exception {
        ...
        hierarchicalShardSyncer
                .checkAndCreateLeasesForNewShardsWithFullShardMap(shardDetector, dynamoDBLeaseRefresher, INITIAL_POSITION_LATEST,
    ...
    /**
     * Test checkAndCreateLeasesForNewShardsWithFullShardMap with a pre-fetched list of shards. In this scenario, shardDetector.listShards()
    ...
    /**
     * Test checkAndCreateLeasesForNewShardsWithFullShardMap with an empty list of shards. In this scenario, shardDetector.listShards()
     * should never be called.
     */
    @Test
    ...
    public void testCheckAndCreateLeasesForNewShardsWhenParentIsOpen() throws Exception {
        ...
        try {
            hierarchicalShardSyncer.checkAndCreateLeasesForNewShardsWithFullShardMap(shardDetector, dynamoDBLeaseRefresher,
    ...
    private void testCheckAndCreateLeasesForNewShardsAndClosedShard(final ExtendedSequenceNumber sequenceNumber,
        ...
        hierarchicalShardSyncer.checkAndCreateLeasesForNewShardsWithFullShardMap(shardDetector, dynamoDBLeaseRefresher, position,
    ...
    private void testCheckAndCreateLeasesForNewShardsAndClosedShardWithDeleteLeaseExceptions(
        ...
        try {
            ...
            hierarchicalShardSyncer
                    .checkAndCreateLeasesForNewShardsWithFullShardMap(shardDetector, dynamoDBLeaseRefresher, position,
                    cleanupLeasesOfCompletedShards, ignoreUnexpectedChildShards, SCOPE);
        } finally {
    ...
    /**
     * Tests that leases are created for both open and closed shards when the lease table is initially empty.
     */
    @Test
    public void testShardFilterUsedWithEmptyLeaseTable() throws Exception {

        final SequenceNumberRange range0 = ShardObjectHelper.newSequenceNumberRange("0", "1");
        final SequenceNumberRange range1 = ShardObjectHelper.newSequenceNumberRange("2", null);

        // Marking shardId-0 as an open shard, and shardId-1 as an open shard.
        List<Shard> shards = Arrays.asList(
                ShardObjectHelper.newShard("shardId-0", null, null, range0,
                        ShardObjectHelper.newHashKeyRange(ShardObjectHelper.MIN_HASH_KEY, "1")),
                ShardObjectHelper.newShard("ShardId-1", null, null, range1,
                        ShardObjectHelper.newHashKeyRange("2", ShardObjectHelper.MAX_HASH_KEY)));
        ...
        hierarchicalShardSyncer
                .checkAndCreateLeasesForNewShardsWithPartialShardMap(shardDetector, dynamoDBLeaseRefresher, INITIAL_POSITION_LATEST,
                        cleanupLeasesOfCompletedShards, ignoreUnexpectedChildShards, SCOPE);

        final Set<Lease> createLeases = new HashSet<>(leaseCreateCaptor.getAllValues());
        final Set<Lease> expectedCreateLeases = new HashSet<>(createLeasesFromShards(shards,
                ExtendedSequenceNumber.LATEST, null));

        assertThat(createLeases, equalTo(expectedCreateLeases));
        verify(shardDetector, times(0)).listShards();
    ...
    private void testCheckAndCreateLeaseForShardsIfMissing(final List<Shard> shards,
        ...
        hierarchicalShardSyncer
                .checkAndCreateLeasesForNewShardsWithFullShardMap(shardDetector, dynamoDBLeaseRefresher, initialPosition,
                cleanupLeasesOfCompletedShards, false, SCOPE);
    ...
    /**
     * Tests that leases for closed shards are not created in non-empty lease table cases.
     */
    @Test
    public void testDetermineNewLeasesToCreateIgnoreClosedShard() {
        final String shardWithLease = "shardId-0";
        final String closedShard = "shardId-1";
        final String openShard = "shardId-2";
        final List<Lease> currentLeases = Arrays.asList(newLease(shardWithLease));

        final List<Shard> shards = Arrays.asList(
                ShardObjectHelper.newShard(shardWithLease, null, null,
                        ShardObjectHelper.newSequenceNumberRange("0", "1")),
                ShardObjectHelper.newShard(closedShard, null, null,
                        ShardObjectHelper.newSequenceNumberRange("2", "3")),
                ShardObjectHelper.newShard(openShard, null, null,
                        ShardObjectHelper.newSequenceNumberRange("4", null)));

        final List<Lease> newLeases = HierarchicalShardSyncer.determineNewLeasesToCreateWithFullShardMap(shards, currentLeases,
                INITIAL_POSITION_LATEST);

        assertThat(newLeases.size(), equalTo(1));
        assertThat(newLeases.get(0).leaseKey(), equalTo(openShard));
    }
